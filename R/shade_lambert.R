...
#' @importFrom raster bandnr crs values<- as.matrix extract extent mask
...
#'
shade_lambert <- function(surface_raster, date, time, tzone, sun_elevation, sun_azimuth,
                          only_facing_sun, z_value, verbose) {
  UseMethod("shade_lambert", surface_raster)
}

#' @export
shade_lambert.RasterLayer <- function(surface_raster, date, time, tzone, sun_elevation, sun_azimuth,
                                      only_facing_sun = FALSE, z_value = 1, verbose = FALSE) {
  ...
  values(shade_raster) <- shadow_matrix

  shade_raster <- mask(shade_raster, surface_raster)
